...
export function GoalGrid({
  ...
  return (
    <div className="mx-auto max-w-4xl animate-fade-up">
      ...
      <div className={hideHeader ? "grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4" : "mt-14 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4"}>
        {goals.map((goal, i) => {
          const Icon = (Icons as unknown as Record<string, Icons.LucideIcon>)[goal.icon] ?? Icons.Box;
          return (
            <Reveal key={goal.id} delay={Math.min(i, 8) * 40}>
              <button
                onClick={() => onSelect(goal.id)}
                className="group flex w-full flex-col items-start gap-4 rounded-xl border border-border bg-surface p-5 text-left transition-all duration-300 hover:-translate-y-1 hover:border-border-strong hover:bg-surface-2"
              >
